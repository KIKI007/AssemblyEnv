    ...
    void readOBJ::loadFromFile(std::string filename) {
        ...
        if (fin.is_open()) {
            ...
            while (std::getline(fin, line)) {
                ...
                {
                    //words[1].pop_back();
                    std::string name = split(words[1], '_')[1];
                    objIDs_.push_back(atoi(name.c_str()));
        ...
        int min_index = *std::min_element(objIDs_.begin(), objIDs_.end());
        for(auto &ind: objIDs_) ind-= min_index;
