...
namespace util
{
    ...
    inline Eigen::Vector3d toVec3(const std::string &pos_text)
    {
    ...
    inline Eigen::Quaternion<double> toQuat(const std::string &quat_text)
    {
        auto quat_data = split(quat_text, ' ');
        Eigen::Quaternion<double> quat;
        for(int id = 0; id < 4; id++) quat.coeffs()[id] = std::atof(quat_data[id].c_str());
        return quat;
    }
