//
// Created by Ziqi Wang on 06.03.2024.
//
...
inline bool list_to_matrix(const std::vector<std::array<T, N> > & V,Eigen::PlainObjectBase<Derived>& M)
{
    // number of rows
    int m = V.size();
    if(m == 0)
...
inline bool is_stl_binary(std::istream &input) {
...
inline bool read_stl_ascii(std::istream &input,
    ...
    auto parse_ascii_normal = [&N](const char *line) {
        ...
        N.push_back({
            {
                static_cast<TypeN>(x), static_cast<TypeN>(y),
                static_cast<TypeN>(z)
            }
        });
        return true;
    ...
    auto parse_ascii_vertex = [&V](const char *line) {
        ...
        V.push_back({
            {
                static_cast<TypeV>(x), static_cast<TypeV>(y),
                static_cast<TypeV>(z)
            }
        });
        return true;
...
template<typename TypeV, typename TypeF, typename TypeN>
inline bool read_stl_binary(std::istream &input,
                            std::vector<std::array<TypeV, 3> > &V,
                            std::vector<std::array<TypeF, 3> > &F,
                            std::vector<std::array<TypeN, 3> > &N) {
    if (!input) {
...
template<typename TypeV, typename TypeF, typename TypeN>
inline bool readSTL(std::istream &input,
                    std::vector<std::array<TypeV, 3> > &V,
                    std::vector<std::array<TypeF, 3> > &F,
                    std::vector<std::array<TypeN, 3> > &N) {
    bool success = false;
